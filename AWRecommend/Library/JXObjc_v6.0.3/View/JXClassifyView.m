...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[JXCell class] forCellReuseIdentifier:[JXCell identifier]];
        _tableView.tableFooterView = [UIView new];
//        _tableView.separatorInset = UIEdgeInsetsZero;
//        _tableView.layoutMargins = UIEdgeInsetsZero;
        _tableView.separatorStyle = UITableViewCellSeparatorStyleNone;
        _tableView.separatorColor = [UIColor clearColor]; // self.selectedBackgroundColor;
...
- (NSString *)ds_identifierForRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([self.dataSource respondsToSelector:@selector(classifyView:identifierForRowAtIndexPath:)]) {
        return [self.dataSource classifyView:self identifierForRowAtIndexPath:indexPath];
    }
    return JXStrWithInt(1298 + indexPath.row);
}

- (UIView *)ds_viewForRowAtIndexPath:(NSIndexPath *)indexPath {
    // NSInteger tag = 1298 + indexPath.row;
    NSString *identifer = [self ds_identifierForRowAtIndexPath:indexPath];
    JXLogDebug(@"identifer = %@", identifer);
    UIView *view = [self.contentView jx_viewWithIdentifier:identifer];
...
//        }
        if ([self.selectedIndexPath compare:indexPath] != NSOrderedSame) {
            self.selectedIndexPath = indexPath;
            [self ds_didChangeAtIndexPath:indexPath forView:view];
        }
    }
...
- (UIView *)mainView {
    return [self ds_viewForRowAtIndexPath:self.selectedIndexPath];
}
...
- (void)ds_didChangeAtIndexPath:(NSIndexPath *)indexPath forView:(UIView *)view {
    if ([self.dataSource respondsToSelector:@selector(classifyView:didChangeAtIndexPath:forView:)]) {
        [self.dataSource classifyView:self didChangeAtIndexPath:indexPath forView:view];
    }
}
...
//    self.selectedIndexPath = indexPath;
    ...
    [tableView scrollToRowAtIndexPath:indexPath atScrollPosition:(animated ? UITableViewScrollPositionMiddle : UITableViewScrollPositionTop) animated:animated];
