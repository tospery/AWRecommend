...
@property (nonatomic, copy) JXStarViewDidSelectBlock didSelectBlock;
@end
...
- (void)custom {
    ...
    _selectedImage = JXImageWithName(@"jxres_rating_on");
    _unselectedImage = JXImageWithName(@"jxres_rating_off");
...
- (void)pressed:(id)sender {
    ...
    if (btn.tag < (_level - 1)) {
        for (int i = (int)(btn.tag + 1); i < _level; ++i) {
        ...
        if (self.didSelectBlock) {
            self.didSelectBlock(self.level);
        }
