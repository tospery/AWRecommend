...
- (void)updateConstraints {
    if (_isButtonPressed) {
        if (_text.length > 0){
            if (JXCollapsibleViewModeClose == _mode) {
                ...
                _textWidth = self.bounds.size.width - kJXStdPadding * 2 - JXCollapsibleViewButtonEdge;
                CGSize requiredSize = [_text jx_sizeWithFont:_font width:_textWidth];
                _matchHeight = requiredSize.height + kJXStdPadding;
            }else if (JXCollapsibleViewModeOpen == _mode) {
                _mode = JXCollapsibleViewModeClose;
                _matchHeight = _baseHeight * _lines + kJXStdPadding;
            }
        }else {
            _matchHeight = 0;
        }
        ...
        if (_text.length > 0) {
            _textWidth = self.bounds.size.width - kJXStdPadding * 2 - JXCollapsibleViewButtonEdge;
            CGSize requiredSize = [_text jx_sizeWithFont:_font width:_textWidth + JXCollapsibleViewButtonEdge];
    ...
    [self mas_updateConstraints:^(MASConstraintMaker *make) {
        make.height.equalTo([NSNumber numberWithFloat:self.matchHeight]).priorityHigh();
    }];
...
- (void)drawRect:(CGRect)rect {
    ...
    if (_text.length > 0) {
        ...
        CGRect textFrame = CGRectMake(kJXStdPadding,
                                      kJXStdPadding / 2.0,
                                      textWidth,
                                      _matchHeight - kJXStdPadding / 2.0);
...
#pragma clang diagnostic pop
        ...
        if (JXCollapsibleViewModeBase != _mode) {
            ...
            //            }
            //
            //            [[UIBezierPath customBezierPathOfArrowSymbolWithRect:CGRectMake(self.bounds.size.width - JXCollapsibleViewButtonEdge - kJXStandardPadding,
            //                                                                            self.bounds.size.height - JXCollapsibleViewButtonEdge - kJXStandardPadding,
...
- (void)setFont:(UIFont *)font {
    ...
    _font = font;
    _baseHeight = [kStringOK jx_sizeWithFont:_font width:UINT16_MAX].height;
}
