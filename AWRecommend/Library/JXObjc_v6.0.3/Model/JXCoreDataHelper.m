...
@interface JXCoreDataHelper ()
@property (nonatomic, readwrite) NSPersistentStore            *store;
@end

@implementation JXCoreDataHelper
#define debug 1

#pragma mark - FILES
NSString *storeFilename = @"AppData.sqlite";
NSString *sourceStoreFilename = @"DftData.sqlite";
...
- (id)init {
    ...
    if (self = [super init]) {
        ...
        [_parentContext performBlockAndWait:^{
            [self.parentContext setPersistentStoreCoordinator:self.coordinator];
            [self.parentContext setMergePolicy:NSMergeByPropertyObjectTrumpMergePolicy];
        }];
        ...
        [_importContext performBlockAndWait:^{
            [self.importContext setParentContext:self.context];
            [self.importContext setMergePolicy:NSMergeByPropertyObjectTrumpMergePolicy];
            [self.importContext setUndoManager:nil]; // the default on iOS
        }];
        ...
        [_sourceContext performBlockAndWait:^{
            [self.sourceContext setMergePolicy:NSMergeByPropertyObjectTrumpMergePolicy];
            [self.sourceContext setParentContext:self.context];
            [self.sourceContext setUndoManager:nil]; // the default on iOS
        }];
    }
    return self;
}
...
- (void)setDefaultDataStoreAsInitialStore {
    ...
    if (![fileManager fileExistsAtPath:self.storeURL.path]) {
        ...
        [NSURL fileURLWithPath:[[NSBundle mainBundle]
                                pathForResource:@"DftData" ofType:@"sqlite"]];
        NSError *error;
        if (![fileManager copyItemAtURL:defaultDataURL
                                  toURL:self.storeURL
                                  error:&error]) {
            NSLog(@"DftData.sqlite copy FAIL: %@",
                  error.localizedDescription);
        }
        else {
...
- (void)deepCopyFromPersistentStore:(NSURL*)url {
    ...
    [_sourceContext performBlock:^{
        ...
        JXCoreDataImporter *importer = [[JXCoreDataImporter alloc]
                                        initWithUniqueAttributes:[self selectedUniqueAttributes]];
        
        [importer deepCopyEntities:entitiesToCopy
                       fromContext:self.sourceContext
                         toContext:self.importContext];
        ...
        [_importContext performBlock:^{
            ...
            for (int a = 1; a < 101; a++) {
                ...
                @autoreleasepool {
                    ...
                    [JXFaulter faultObjectWithID:photo.objectID
                                       inContext:self.importContext];
                    [JXFaulter faultObjectWithID:item.objectID
                                       inContext:self.importContext];
                }
            }
 ...
 qualifiedName:(NSString *)qName
    ...
    [self.importContext performBlockAndWait:^{
        ...
        if ([elementName isEqualToString:@"item"]) {
            ...
            JXCoreDataImporter *importer =
            [[JXCoreDataImporter alloc] initWithUniqueAttributes:
            ...
            [JXCoreDataImporter saveContext:self.importContext];
            
            // STEP 7: Turn objects into faults to save memory
            [JXFaulter faultObjectWithID:item.objectID inContext:self.importContext];
            [JXFaulter faultObjectWithID:unit.objectID inContext:self.importContext];
            [JXFaulter faultObjectWithID:locationAtHome.objectID inContext:self.importContext];
            [JXFaulter faultObjectWithID:locationAtShop.objectID inContext:self.importContext];
        }
    }];
}
...
//    static JXCoreDataHelper *instance;
//    static dispatch_once_t onceToken;
//    dispatch_once(&onceToken, ^{
//        instance = [[JXCoreDataHelper alloc] init];
