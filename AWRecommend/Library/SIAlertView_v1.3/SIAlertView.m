...
#define MESSAGE_MAX_LINE_COUNT 12 // YJX_CUSTOM
#define GAP 10
...
@property (nonatomic, strong) UIScrollView *messageScrollView;// *messageLabel; // YJX_CUSTOM
...
- (void)validateLayout
{
    ...
    if (self.messageScrollView) {
        ...
        CGFloat height = [self scrollHeightForScrollView];
        self.messageScrollView.frame = CGRectMake(CONTENT_PADDING_LEFT, y, self.containerView.bounds.size.width - CONTENT_PADDING_LEFT * 2, height);
        y += height;
        
        CGSize size = CGSizeMake(self.messageScrollView.jx_width, [self contentHeightForScrollView]);
        self.messageScrollView.contentSize = size;
...
- (CGFloat)preferredHeight
{
    ...
    if (self.message) {
        ...
        height += [self scrollHeightForScrollView];
    }
    if (self.items.count > 0) {
        if (height > CONTENT_PADDING_TOP) {
            height += GAP;
...
- (CGFloat)scrollHeightForScrollView {
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
        CGSize size = [self.message sizeWithFont:self.messageLabel.font
                               constrainedToSize:CGSizeMake(CONTAINER_WIDTH - CONTENT_PADDING_LEFT * 2, maxHeight)
                                   lineBreakMode:/*self.messageLabel.lineBreakMode*/NSLineBreakByWordWrapping]; // YJX_CUSTOM
#pragma clang diagnostic pop

        
//        CGSize size = [TTTAttributedLabel sizeThatFitsAttributedString:self.messageLabel.attributedText withConstraints:CGSizeMake(CONTAINER_WIDTH - CONTENT_PADDING_LEFT * 2, maxHeight) limitedToNumberOfLines:MESSAGE_MAX_LINE_COUNT];
...
- (CGFloat)contentHeightForScrollView {
    CGFloat minHeight = MESSAGE_MIN_LINE_COUNT * self.messageLabel.font.lineHeight;
    if (self.messageLabel) {
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
        CGSize size = [self.message sizeWithFont:self.messageLabel.font
                               constrainedToSize:CGSizeMake(CONTAINER_WIDTH - CONTENT_PADDING_LEFT * 2, INT32_MAX)
                                   lineBreakMode:/*self.messageLabel.lineBreakMode*/NSLineBreakByWordWrapping]; // YJX_CUSTOM
#pragma clang diagnostic pop
...
//#endif
            self.messageScrollView = [[UIScrollView alloc] initWithFrame:self.bounds];
            self.messageScrollView.contentSize = self.bounds.size;
            [self.containerView addSubview:self.messageScrollView];
        ...
        [self.messageLabel removeFromSuperview];
        self.messageLabel = nil;
        [self.messageScrollView removeFromSuperview];
        self.messageScrollView = nil;
    }
    [self invalidateLayout];
}
