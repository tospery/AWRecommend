...
- (void)config {
	...
	UIImageView *topLine = [[UIImageView alloc] initWithFrame:CGRectMake(0, -5, JXScreenWidth, 5)];
...
- (void)setTabBarItemAttributes:(NSArray<NSDictionary *> *)tabBarItemAttributes {
    ...
    CGFloat normalItemWidth = (JXScreenWidth * 3 / 4) / (_tabBarItemAttributes.count - 1);
    CGFloat tabBarHeight = CGRectGetHeight(self.frame);
    CGFloat publishItemWidth = (JXScreenWidth / 4);
...
- (LLTabBarItem *)tabBarItemWithFrame:(CGRect)frame title:(NSString *)title normalImageName:(NSString *)normalImageName selectedImageName:(NSString *)selectedImageName tabBarItemType:(LLTabBarItemType)tabBarItemType {
    ...
    item.titleLabel.font = JXFont(8); // [UIFont systemFontOfSize:8];
    ...
    [item setTitleColor:JXColorHex(0x333333) forState:UIControlStateNormal];
    if (tabBarItemType == LLTabBarItemNormal) {
        [item setTitleColor:kColorGreenDark forState:UIControlStateSelected];
    }
