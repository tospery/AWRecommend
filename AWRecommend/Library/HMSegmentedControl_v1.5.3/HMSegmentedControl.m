...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
		...
		[self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            CGFloat space = JXAdaptScreen(6.0);
            
            UIImage *icon = iconImage;
            CGFloat imageWidth = JXAdaptScreen(30); // icon.size.width;
            CGFloat imageHeight = JXAdaptScreen(30); // icon.size.height;
            ...
            CGFloat imageYOffset = roundf((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight - imageHeight - stringHeight - space) / 2.0f);
            
            CGFloat yOffset = roundf(imageYOffset + imageHeight + space);
    ...
    }else if (self.type == HMSegmentedControlTypeTextImagesLeft){
        [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            ...
            CGFloat imageXOffset = self.segmentEdgeInset.left; // Start with edge inset
            CGFloat textXOffset  = self.segmentEdgeInset.left;
            CGFloat textWidth = [self measureTitleAtIndex:idx].width;
...
//                textWidth = self.segmentWidth;
                ...
                imageXOffset = (self.segmentWidth * idx) + (self.segmentWidth - imageWidth - padding - textWidth) / 2.0f;
                textXOffset = imageXOffset + imageWidth + padding;
            } else if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
                // When we are drawing dynamic widths, we need to loop the widths array to calculate the xOffset
                CGFloat xOffset = 0;
                NSInteger i = 0;
            ...
            CGFloat imageYOffset = roundf((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2.0f - (imageHeight / 2.0f));
            CGRect imageRect = CGRectMake(imageXOffset, imageYOffset, imageWidth, imageHeight);
            CGRect textRect = CGRectMake(textXOffset, yOffset, textWidth, stringHeight);
...
- (NSUInteger)sectionCount {
    if (self.type == HMSegmentedControlTypeText) {
        return self.sectionTitles.count;
               ...
               self.type == HMSegmentedControlTypeTextImages ||
               self.type == HMSegmentedControlTypeTextImagesLeft) {
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(enlargeRect, touchLocation)) {
        ...
        } else if (self.type == HMSegmentedControlTypeTextImages || self.type == HMSegmentedControlTypeText || self.type == HMSegmentedControlTypeTextImagesLeft) {
