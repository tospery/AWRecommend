...
@interface FilterViewController () <UITableViewDataSource, UITableViewDelegate, JXClassifyViewDataSource>
...
- (RACSignal *)requestRemoteDataSignalWithPage:(NSInteger)page {
    return [HRInstance queryDiseaseBySuitObjectId:[self.classify.jxID integerValue]];
...
//        }
//    }
    for (NSString *t in self.selectedTypes) {
        [ms appendString:t];
        [ms appendString:@" "];
    }
...
//    return d.datas.count;
    ...
    FilterSymptom *c = self.dataSource[self.classifyView.selectedIndexPath.row];
...
//    cell.data = d2;
    ...
    NSString *name = c.disease[indexPath.row];
    cell.data = RACTuplePack(@1, name, @([self.selectedTypes containsObject:name]));
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.selectedTypes containsObject:name]) {
        [self.selectedTypes removeObject:name];
    }else {
        [self.selectedTypes addObject:name];
    }
